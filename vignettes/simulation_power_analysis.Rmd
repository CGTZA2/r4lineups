...
Power analysis for comparing two different discriminability levels:

```{r power_comparison}
# Power to detect difference between d' = 1.5 and d' = 2.0
power_diff <- simulate_power_analysis(
  sample_sizes = c(100, 150, 200, 250, 300),
  d_prime = c(1.5, 2.0),  # Two conditions
...
### Custom Effect Measure

You can use different effect measures for power analysis:

```{r power_custom}
# Power analysis using suspect ID rate
power_id_rate <- simulate_power_analysis(
  sample_sizes = seq(50, 300, by = 50),
  d_prime = 1.5,
  effect_measure = "suspect_id_rate",
  n_simulations = 100
)

# Power analysis using diagnosticity ratio
power_diag <- simulate_power_analysis(
  sample_sizes = seq(50, 300, by = 50),
  d_prime = 1.5,
  effect_measure = "diagnosticity",
  n_simulations = 100
)
```
...
Research question: Do simultaneous lineups provide better discriminability than sequential lineups?

```{r planning_example}
# Assumptions:
# - Simultaneous: d' = 1.8
# - Sequential: d' = 1.5
# - We want 80% power to detect this difference
...
# Result: We need approximately 150 participants per condition for 80% power
```
...
roc_small <- make_roc(small_sample)
