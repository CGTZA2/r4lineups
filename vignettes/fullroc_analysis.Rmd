...
**Caution — optimistic bias.** Because the ordering is chosen from the *sample*
diagnosticity ratios, the curve is bowed upward by noise alone: even identical
target-present and target-absent response distributions produce an AUC slightly
above 0.5. The bias grows with the number of decision × confidence cells and
shrinks with sample size. Diagnosticity ordering is useful for exploration, but
for inference or comparisons across conditions prefer a-priori ordering (below)
or benchmark against a permutation baseline.
...
Because the ordering is fixed in advance rather than estimated from the data,
a-priori ordering does not suffer from the optimistic bias described above and
is the safer choice for hypothesis tests and between-condition comparisons
(this is why Smith & Yang, 2020, offer it).
...
1. **Use full ROC when the inferential target is total evidentiary value**
...
2. **Pre-specify ordering for confirmatory comparisons**
   ...
   - Diagnosticity ordering is useful as an exploratory upper-bound estimate,
     but its AUC is optimistically biased and can be unstable across samples
