...
**Returns**: List with `$auc`, `$roc_data`, `$diagnosticity_table`,
`$plot`, `$summary`
...
- `"diagnosticity"` (default, recommended): Order by empirical DR =
  HR/FAR
  - Data-driven
...
**When to use each**: - **Full ROC** (`make_fullroc`): New analyses
(recommended) - **Partial ROC** (`make_roc`): Replicating older studies
